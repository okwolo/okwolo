...
window.goo = function (controllers, args, options) {
    // input validation
    inputValidation();
    ...
    var statePersistenceManager = dictTimeMachine(args.state, options);
    ...
    var stateManager = dict(args.actions, args.middleware, args.watchers, options);

    // replacing the act function to pass state to it
    var statelessAct = stateManager.act;
    stateManager.act = function (type, params) {
    ...
    function dict(actionTypes, middleware, watchers, options) {
        ...
        function act(state, type, params) {
            ...
            middleware.reverse().forEach(function (currentMiddleware, index) {
                funcs[index + 1] = function (state, type, params) {
                    var m = currentMiddleware(funcs[index], JSON.parse(JSON.stringify(state)), type, params);
                    return m;
                };
            });
            var newState = JSON.parse(JSON.stringify(funcs[middleware.length](state, type, params)));
        ...
        function execute(state, type, params) {
            actionTypes.forEach(function (currentActionTypes, i) {
                var action = currentActionTypes[type];
                if (!action) {
                    return;
                }
                action.forEach(function (currentAction) {
    ...
    function gooey(target, build, initialState) {
        ...
        function update(newState) {
            window.requestAnimationFrame(function () {
                return _update(vdom, build(newState), {});
            });
            /**
        ...
        function diff(original, successor) {
            // get types
            var originalType = Object.prototype.toString.call(original);
            var successorType = Object.prototype.toString.call(successor);
            ...
            if (originalType === '[object Object]' || originalType === '[object Array]') {
                ...
                var newKeys = Object.keys(successor);
                // creating union of both arrays of keys
                if (originalType === '[object Array]') {
                    var lengthDifference = newKeys.length - keys.length;
                    if (lengthDifference > 0) {
                        for (var i = lengthDifference; i > 0; --i) {
    ...
    function inputValidation() {
        ...
        if (!args.watchers.reduce(function (a, w) {
            return a && typeof w === 'function';
        }, true)) {
        ...
        if (!args.middleware.reduce(function (a, w) {
            return a && typeof w === 'function';
        }, true)) {
        ...
        args.actions.forEach(function (currentActionTypes, i) {
            Object.keys(currentActionTypes).forEach(function (currentActionType, j) {
                ...
                currentActionTypes[currentActionType].forEach(function (currentAction, k) {
                    ...
                    currentAction.target.forEach(function (key, l) {
