...
const draw = (global) => (target, newVDOM) => {
...
const updateElement = (global) => (original, successor, parent, parentKey) => {
    ...
    for (let i = 0; i < childKeys.length; ++i) {
        ...
        updateElement(global)(original.children[key], successor.children[key], original, key);
    }
...
const update = (global) => (target, newVDOM, VDOM) => {
    ...
    global.requestAnimationFrame(() => {
        try {
            updateElement(global)(VDOM, newVDOM, {DOM: target, children: {root: VDOM}}, 'root');
        } catch (e) {
